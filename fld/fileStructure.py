...
def fileIsDuplicate(file_name):
    for file in FILES:
        if file[2] == os.path.abspath(file_name):
            return True
    return False
        ...
        if(file == "Makefile"):
            with open(folder + "/" + file, "r") as make:
                for make_line in make:
                    ...
                    if make_line.startswith("HEADERS :="):
                        headers = make_line.split(" := ")[1].split("$")
                        for header in headers:
                            if header.startswith("(wildcard "):
                                hfiles = header.split(" ")[1].replace(")", "")
                                hfiles = sorted(glob.iglob(folder + "/" + hfiles, recursive=False))
                                for hfile in hfiles:
                                    FILES.append([type, [folder, os.path.basename(folder), []], os.path.abspath(hfile)])
                    if make_line.startswith("CSOURCES :="):
                        csources = make_line.split(" := ")[1].split("$")
                        for csource in csources:
                            if csource.startswith("(wildcard "):
                                cfiles = csource.split(" ")[1].replace(")", "")
                                cfiles = sorted(glob.iglob(folder + "/" + cfiles, recursive=False))
                                for cfile in cfiles:
                                    FILES.append([type, [folder, os.path.basename(folder), []], os.path.abspath(cfile)])
                    if make_line.startswith("CPPSOURCES :="):
                        cppsources = make_line.split(" := ")[1].split("$")
                        for cppsource in cppsources:
                            if cppsource.startswith("(wildcard "):
                                cppfiles = cppsource.split(" ")[1].replace(")", "")
                                cppfiles = sorted(glob.iglob(folder + "/" + cppfiles, recursive=False))
                                for cppfile in cppfiles:
                                    FILES.append([type, [folder, os.path.basename(folder), []], os.path.abspath(cppfile)])
                    if make_line.startswith("CCSOURCES :="):
                        ccsources = make_line.split(" := ")[1].split("$")
                        for ccsource in ccsources:
                            if ccsource.startswith("(wildcard "):
                                ccfiles = ccsource.split(" ")[1].replace(")", "")
                                ccfiles = sorted(glob.iglob(folder + "/" + ccfiles, recursive=False))
                                for ccfile in ccfiles:
                                    FILES.append([type, [folder, os.path.basename(folder), []], os.path.abspath(ccfile)])
