...
struct SyscallHandler : public sc_core::sc_module, syscall_emulator_if {
	...
	/* Modification chazott for splitted instruction and data memory */
	uint8_t *mem_data; 
	uint64_t mem_data_offset = UINT64_MAX;
	/* Modification end */
	...
	void init(uint8_t *host_memory_pointer, uint64_t mem_start_address, uint64_t heap_pointer_address, uint64_t heap_size) {
	...
	/* Modification chazott for splitted instruction and data memory */
	void init(uint8_t *host_memory_pointer, uint64_t mem_start_address, uint8_t *host_data_memory_pointer, uint64_t mem_data_start_address, uint64_t heap_pointer_address, uint64_t heap_size) {
		mem = host_memory_pointer;
		mem_offset = mem_start_address;
		mem_data = host_data_memory_pointer;
		mem_data_offset = mem_data_start_address;
	...
	uint8_t *guest_address_to_host_pointer(uintptr_t addr) {
		...
		if(addr < mem_data_offset) {
			return mem + (addr - mem_offset);
		}else {
			return mem_data + (addr - mem_data_offset);
		}
		//return mem + (addr - mem_offset);
		/* Modification end */
