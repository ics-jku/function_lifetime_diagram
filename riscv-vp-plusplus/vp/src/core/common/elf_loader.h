...
struct GenericElfLoader {
	...
	void load_executable_image(load_if &load_if, addr_t size, addr_t offset, bool use_vaddr = true) {
		for (auto p : get_load_sections()) {
			...
			if (addr + p->p_memsz >= offset + size) {
				std::cout << "Addr+ p->p_memsz: " << addr + p->p_memsz << " offset + size: " << offset + size << std::endl;
				std::cerr << "[elf_loader] ";
			...
			//std::cout << "Address: " << std::hex << addr << " Offset: " << std::hex << offset << std::endl; 
			
			const char *src = elf.data() + p->p_offset;
			//std::cout << "elf.data(): " << std::hex << elf.data() << " POffset: " << std::hex << p->p_offset << std::endl;
			...
			idx = idx + p->p_filesz;
			//std::cout << "MemsSize: " << std::hex << p->p_memsz << " FileSize: " << std::hex << p->p_filesz << std::endl; 
	...
	addr_t get_heap_addr() {
		auto heap = get_section(".heap");
		return heap->sh_addr;
	}

	addr_t get_heap_size() {
		auto heap = get_section(".heap");
		return heap->sh_size;
	}
