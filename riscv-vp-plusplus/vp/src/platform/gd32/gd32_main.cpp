...
class GD32Options : public Options {
   public:
	...
	addr_t sram_size 		= 1024 * 32768;  // 32 MB sram --> Changed for coverage
	addr_t sram_start_addr 		= 0x20000000;
	addr_t sram_end_addr = sram_start_addr + sram_size - 1;

	addr_t flash_size 		= 1024 * 1024;  // 128 KB flash	--> Changed to 256 for coverage
...
int sc_main(int argc, char **argv) {
...

	{
		...
		ahb.ports[it++] = new PortMapping(opt.sys_start_addr, opt.sys_end_addr);
	}
	...
	sys.init(flash.data, opt.flash_start_addr, sram.data, opt.sram_start_addr, loader.get_heap_addr(), loader.get_heap_size());
