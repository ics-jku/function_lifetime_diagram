...
public:
	...
	addr_t mems_gyro_start_addr = 0x50000000;
	addr_t mems_gyro_end_addr = 0x50000fff;
	addr_t basic_timer_start_addr = 0x50001000;
	addr_t basic_timer_end_addr = 0x50001fff;
...
int sc_main(int argc, char **argv) {
	...
	sys.init(mem.data, opt.mem_start_addr, loader.get_heap_addr(), 0);
	...
	// address mapping
	{
		...
		bus.ports[it++] = new PortMapping(opt.mems_gyro_start_addr, opt.mems_gyro_end_addr);
		bus.ports[it++] = new PortMapping(opt.basic_timer_start_addr, opt.basic_timer_end_addr);
...

	{
		...
		bus.isocks[it++].bind(mems_gyro.tsock);
		bus.isocks[it++].bind(timer.tsock);
